...
namespace Biometrics
{
	...
	public class CFingerPrint 
	{
		...
		{
			Bitmap m_ImageBuffer = new Bitmap(FP_IMAGE_WIDTH,FP_IMAGE_HEIGHT );
			for (int i = 0; i<= FP_IMAGE_WIDTH - 1;i++)
			{
				for (int j = 0;j<= FP_IMAGE_HEIGHT - 1;j++)
				{
					if (P[i,j] == 1)
						m_ImageBuffer.SetPixel(i,j,Color.Blue);
					else
						m_ImageBuffer.SetPixel(i,j,Color.White);
				}
			}
		...
		public Bitmap getFingerPrintImageDetail()
		{
			...
			Graphics gf = Graphics.FromImage(m_ImageBuffer);
